...
- A hero screenshot of the Collect tab showing the NFC scan prompt (place in `src/images/` and embed near the Features section).
- A snapshot of the Upload Data table and sync status icons (place in `src/images/` and embed in the Features or Project Structure section).

## Notes for field use
- NFC and geolocation functionality require a physical device; browser previews will only demonstrate the UI states.
- When packaging for farmers, ensure offline handling and sync retries are implemented for low-connectivity environments.
